...
- **诉求（用户原话）**：进场动画停下后**静止要方方正正**，鼠标移上去才**浮起/按压**（pen 的手感）；"你上次丑是停下来后不是铺平的方方正正，而是斜七竖八乱七八糟"。
- **两次尝试均被否决并回退** ✗：① 把两块面板的阴影重写为分层黑 + 弱光 —— 判"丑、比原来还差"；② 三块面板 `baseRotate*` 归零 + `forwards → backwards` —— 真机现象"**进场停下后突然卡片变大一圈变模糊**，其余啥也没有"。
