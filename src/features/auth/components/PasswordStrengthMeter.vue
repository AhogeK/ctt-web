...
const strengthColor = computed(() => {
  ...
  if (score.value <= 2) return 'bg-destructive'
  if (score.value <= 5) return 'bg-warning-solid'
  return 'bg-success'
})
...
const glowColor = computed(() => {
  if (!hasPassword.value) return ''
  if (score.value <= 2) return 'shadow-destructive/40'
  if (score.value <= 5) return 'shadow-warning-solid/40'
  return 'shadow-success/40'
})
...
function iconColor(rule: Rule): string {
  if (!hasPassword.value) return 'text-muted-foreground dark:text-muted-foreground'
  return rule.passed ? 'text-success dark:text-success' : 'text-muted-foreground dark:text-muted-foreground opacity-50'
}

/** Text color for a rule - neutral gray when empty, active colors when has password */
function textColor(rule: Rule): string {
  if (!hasPassword.value) return 'text-muted-foreground dark:text-muted-foreground'
  return rule.passed ? 'text-success dark:text-success' : 'text-muted-foreground dark:text-muted-foreground opacity-60'
}
</script>
...
<template>
  <div class="mt-0 space-y-3">
    ...
    <div class="h-1.5 w-full overflow-hidden rounded-full bg-muted dark:bg-white/6">
      <div
    ...
    <div
      class="text-[11px] text-muted-foreground dark:text-muted-foreground transition-opacity duration-200"
    ...
    <ul class="space-y-2">
      <li
        ...
        :key="index"
        class="flex items-center gap-2 text-[11px] leading-relaxed transition-colors duration-200"
