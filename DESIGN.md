...
| Kinematics | one rAF integrator for angles *and* lift: `clamp((target-current)*smoothness, ±0.65°/frame)` plus a fixed `+0.0035/frame` on the scale | a CSS transition on `transform` is restarted by every pointermove and starts again from zero velocity, so the card never leaves the flat start of the curve — the rubber-band trail. Both dimensions must ride the same clock, or the card inflates before it tilts and reads as a stretched edge rather than a lift |
| Pointer field | normalised to the unit disk (a corner is `sqrt(2)` in a box) | without it a corner asks for `sqrt(5.5²+5.5²) = 7.8°`: the first place a projection visibly distorts both axes at once |
| Perspective | inlined per card as `perspective(1200px)` | the container carried `perspective-origin: 50% 30%`, so every card was projected off its own axis |
