...
- **校准**：重大改造或复盘后，回看领域文件是否已漂移 —— 漂移靠主动看发现，不是等谁撞上过时指引。**本仓库的具体触发**：`../ctt-server` 的 `appVersion`（`gradle/libs.versions.toml`）变化后，复核 `backend-contract` 与 `achievements` 中钉住旧版本的契约事实。
