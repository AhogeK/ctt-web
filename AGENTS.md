...
`src/layouts/AuthLayout.css`（>1700 行）**只许**「追加独立模块」或「精确整串替换」✗ 禁止任何**删除正则/批量清洗** ✗。
**每次改完立即构建**（构建是唯一能证明文件未被破坏的检查）。
**事故记账**：两次用删除正则清洗同一文件 ⇒ ① 静默吃掉 41 行"值换行"声明的属性名（`font-family:` / `transition:` / `box-shadow:`）② 把 1783 行削到 **187 行**。恢复只能靠 `git show HEAD:<file> > <file>` 再逐字写回 ✓。

### R28: 像素级测量的对照纪律（强制）

**差分前必须冻结几何**：悬停会改 `transform`（缩放/旋转）⇒ 卡内所有像素**位移** ⇒ 朴素"悬停 vs 空闲"差分量到的是位移不是光 ✗（实例：A3 读 25.3 counts"卡面穿透"，冻结 `transform` 后为 **3.7** ⇒ 结论反转）。
操作：注入 `transform: none !important; transition: none !important` 后再差分；**任何"光/颜色穿透"结论都必须先过这一关**。

### R29: 分支差距的口径（强制）

**cherry-pick 流程下禁止引用 `rev-list --count A..B`** ✗ —— 被 pick 的提交在新分支上是**新 SHA** ⇒ 原始提交永远算"缺失" ⇒ 数字严重虚高且**看起来合理**（实例：真实内容差距 11 文件 / +1192 行，SHA 口径报 **909** ✗）。
**引用差距只许用内容口径** ✓：`git diff --stat <目标> <来源> -- <非AI路径>`；**提交清单只用来定位"由谁携带内容"** ✓。
**工具**：`bash ~/.omp/packages/session-discipline/tools/branch-gap.sh [目标] [来源]` ✓ —— 依次给：① 祖先关系判定（合并流程下计数有效 ✓）② **内容差异（权威 ✓）** ③ 补丁指纹待办（`git cherry -v` ✓ 并按 AI/非 AI 拆分 ✓，因为 AI 记忆提交的补丁永远不会相同 ✓）。

### R30: 已接受 = 冻结（强制）

用户目视确认过"没问题"的视觉项（尺寸 · 间距 · 色彩 · 手感），**不得**再当成"发现"重报一遍 ✗ ——
重新量到同样的数字、再问"要不要改"，是在重复**用户已经拒绝过**的工作 ✓。
**红线**：① 不得把这些项搭进无关改动里顺手"修" ✗；② 确有必要（例如另一处改动必然影响它）⇒ **明说并重新征询** ✓；
③ 判过的结论必须有落地位置（`.omp/qa/visual-qa.md` 的结论档 + 对应领域 ✓），否则下个会话无从知道它已被接受 ✗。
> 出处：2026-09-24 五项目视验收全过 ⇒ 冻结；实例：手机端 h1 的 64px 与眉题 4.24/4.42 **均已确认不改** ✓。
