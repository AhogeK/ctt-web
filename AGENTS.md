...
- **分层分工**：横切规范（命名、组件架构、错误处理）留在 `systemPatterns.md`；领域专属判断进领域文件，**不得两处重复**（重复即需合并并互相引用）。
...
`memory-bank/domains/<domain>/` — 每领域五件套 `meta.md`（边界）/ `principles.md`（不变量）/ `scenarios.md`（触发→判断→动作）/ `practices.md`（做法与坑）/ `references.md`（事实查表）；入口 `domains/README.md`。
