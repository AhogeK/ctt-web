...
- [x] **v0.12.0** API Key Create Flow (M2) — CreateApiKeyDialog (vee-validate form, scope mode toggle, expiration presets + custom date, 409 AUTH_014 inline banner), RawKeyDialog one-time display (overlay/Esc/X blocked, copy-gated close button, three-tier clipboard fallback, alertdialog a11y), `createApiKey()` API + `useCreateApiKey` mutation, `useCopyToClipboard` composable, `expiresAt` future-time Zod refine, AUTH_014/AUTH_010 error mappings. 5 new files, 956/956 tests pass (+28).
