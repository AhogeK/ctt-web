# Archive: dashboard panel era (v0.28 – v0.37)

Frozen on 2026-09-15 (at v0.43.0) out of `activeContext.md`, which was at its 200-line limit and was
holding a version history that `progress.md` already carries.

**Before archiving, the judgement in here was rescued into the domain layer** — four rules existed
*only* in the timeline file, where they would have been lost:

| Rescued rule | Now lives in |
| ------------ | ------------ |
| Only page-level state goes in the URL (panel windows stay local) | `dashboard-visualization/principles.md` P9 |
| Panel order is the template; the E2E spec pins render order | `dashboard-visualization/practices.md` |
| Truncation is measured (`scrollWidth`), never predicted from character counts | `dashboard-visualization/practices.md` |
| Segment geometry uses the exact share; only the readout is formatted | `dashboard-visualization/practices.md` |

Everything else here is history: what changed and why, per version.
