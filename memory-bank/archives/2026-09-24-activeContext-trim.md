# activeContext 裁剪归档 — 2026-09-24

> 从 [`../activeContext.md`](../activeContext.md) 移出（该文件只留「现在与最近」✓ R24）。
> **本文件是时间线快照，不是入口** ✗ —— 其中的耐久判断已按 R24 回迁领域：面板动效/静止契约 → `domains/auth-layout/` ✓ ·
> hero 动效与 P1 语义 → `domains/landing-page/` ✓ · 记忆与提交流程 → `domains/ai-workflow/` ✓。
> 裁掉时逐条核对过"领域有没有承接"✓ —— 缺的三条（运动学时钟 · 各向同性半径 · 静止契约根因）已先补进 `auth-layout/practices.md` ✓。
> 两条零散教训（dev server `504 Outdated Optimize Dep` 的处理、图表设计依据不做 lieflat 移植）只在此存档 ✓（领域两处已近 200 行上限 ✗）。
