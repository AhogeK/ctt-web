...
| API Key Management      | API Keys list view at `/settings/api-keys` with GitHub PAT-style table, status badges, scope chips, relative time; create flow with one-time raw key display (copy-gated close, hard-to-dismiss dialog); revoke flow with AlertDialog confirmation, idempotent DELETE + BOLA-safe AUTH_010 handling; error mapping (key-limit banner, rate-limit countdown, generic BOLA message), first-load skeleton anti-flicker, table a11y, header shortcut |
