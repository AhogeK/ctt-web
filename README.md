...
| Landing Page            | Public `/` (no session required) rendering `LandingView` under `MarketingLayout` — top bar + content + footer, no sidebar and no user menu; the top-bar CTA follows auth state (register when signed out, dashboard when signed in); own `feature-landing` build chunk; `/` is deliberately **not** `guestOnly`, so a signed-in visitor stays on the marketing page instead of being bounced to the dashboard; section primitives live in `LandingSection` with one spacing/type scale — the display/heading ladder is `@theme` tokens taken verbatim from `DESIGN.md` §3, `--marketing-header-height` is the single sticky offset, and a global `prefers-reduced-motion` rule covers every animation |
| Theme System            | Light/dark mode toggle, Pinia theme store with VueUse persistence; **zero-flash first paint** via an external `public/theme.js` bootstrap placed in `<head>` before the app module (the CSP is `script-src 'self'`, so it cannot be inlined) — it reads the `vueuse-color-scheme` store and syncs `style.colorScheme` so native scrollbars and form controls follow the theme |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
