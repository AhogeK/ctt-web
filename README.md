...
| Achievements            | `/achievements` trophy cabinet — one card per **trophy family** (the API returns 15 badges that are 7 families of 2–3 tiers sharing one measured value, so 15 cards would repeat the same number five times); each card shows inline SVG artwork whose paint rises through the existing indigo ladder (`#3d49ad` → `#8290f0` → `#b9c1ff`, plus a ring when maxed) with no second palette and no image assets, the earned tier's name, a rung strip for the ladder's shape, and progress measured **between the current and next rung** (a bar against the current rung alone would read 100% the moment it unlocked); header totals count tiers (`8 / 15 · 53%`); cards ordered nearest-to-next-rung first with completed ladders last, compared as a unit-free fraction because raw distances mix days, hours and months; a badge the build does not recognise still renders as its own single-tier trophy rather than being dropped, so a server-side addition needs no frontend change (grouping lands in `trophy-model.ts`) |
...
```text
src/
├── features/       # Feature modules (achievements/, auth/, dashboard/, devices/, leaderboard/, settings/)
